...
from utils.servicehelper import returnDataInFormat, RedisCache, collectPredictionBlocksForRoi
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--sigmas', type=float, action='append', default=None, help='smoothing sigmas, defaults to 1 in each spatial dimension')
    ...
    if options.sigmas is None:
        options.sigmas = [1.0] * dim
    else:
        assert len(options.sigmas) == dim, "Number of smoothing sigmas must match the dimensionality of the data"
    presmoothingSigmas = options.sigmas
